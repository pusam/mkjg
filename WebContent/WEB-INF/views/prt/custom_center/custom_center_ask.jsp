<%@page import="kr.co.sist.mkjg.protector.vo.SelCntVO"%>
<%@page import="kr.co.sist.mkjg.protector.vo.IdVO"%>
<%@page import="kr.co.sist.mkjg.protector.domain.AskList"%>
<%@page import="java.util.List"%>
<%@page import="kr.co.sist.mkjg.protector.dao.CustomCenterDAO"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%
	request.setCharacterEncoding("UTF-8");
%>
<jsp:useBean id="scvo" class="kr.co.sist.mkjg.protector.vo.SelCntVO" scope="page"/>
...
$(function(){
	$("#btnAsk").click(function(){
		...
		location.href="custom_center_ask_write.do";		
	});
...
#ask_list{margin-left:350px;margin-top:70px;}

#frm{padding-left:20px;width:800px;text-align:center;}
#tab{width:730px;border-spacing:0px;}
...
#btnAsk{width:70px;height:30px;font-family:'Ubuntu'; font-size:13px;background-color:#F2F2EF; border:1px solid #ccc;float:right;}
    ...
    <body>
        <div id="page">
            ...
            <section class="blog">
                <div class="container">
                    <div class="row">
                    	<div id="sub_title_top"><img src="http://211.63.89.152/mkjg/common/prt/images/mini_home.png" style="margin-bottom:3px;"/> &gt; 고객센터 &gt; 문의하기</div>
                    	<div id="sub_title"><div id="move_title"><img src="http://211.63.89.152/mkjg/common/prt/images/icons/q&a.png" style="width:60px;"/>&nbsp; 문의하기</div></div>
                     	<div id="ask_list">
						...
						<form id="frm" method="get"> 
							<table id="tab">
								<thead>
								<tr>
									<th id="numTitle" style="width:50px;">번호</th>
									<th id="subTitle" style="width:480px;">제목</th>
									<th id="askStuTitle" style="width:80px;">답변상태</th>
									<th id="inputTitle" style="width:80px;">작성일</th>
								</tr>
								</thead>
								<tbody>
								<c:forEach var="ask" items="${ask_list}">
									<c:set var="i" value="${i+1 }"/>
									<tr>
										<td><c:out value="${selectAskCnt+1-(current_page-1)*ps -num }"/></td>
										<td><a href="custom_center_ask_read.jsp?num=${num}&current_page=${current_page}"><c:out value="${ask.qtitle}"/></a></td>
										<td><c:out value="${ask.rstatus}"/></td>
										<td><c:out value="${ask.qdate}"/></td>
									</tr>
								</c:forEach>
								...
								<tr>
									<td colspan="4" style="height:50px;">
										<input type="button" id="btnAsk" value="문의하기"/>
									</td>
								</tr>
								<tr>
									...
									<td colspan="4" style="border-top:1px solid white;">
										<div id="search">
											<select style="height:23px;">
												<option>-----선택-----</option>
												<option value="qtitle" ${param.fieldName eq 'qtitle'?" selected='selected'":"" }>제목</option>
												<option value="qstauts" ${param.fieldName eq 'qstauts'?" selected='selected'":"" }>답변상태</option>
												<option value="qdate" ${param.fieldName eq 'qdate'?" selected='selected'":"" }>작성일</option>
											</select>
											<input type="text" name="keyword" id="keyword" value="${param.keyword }" maxlength="50" style="width:230px;"/>
                        ...
                        <aside class="col-md-3 col-sm-4 col-xs-12">
                            <div class="blog-list">
                                <h3><img src="http://211.63.89.152/mkjg/common/prt/images/icons/customer_center.png" style="width:35px;"/><font style="margin-left:10px;">고객센터</font></h3>
                                <ul>
