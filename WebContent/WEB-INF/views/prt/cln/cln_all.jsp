...
$(function(){
	...
	$("#search_all").click(function() {
		...
		if(sigungu!="none"){
			$.ajax({
				url:"result_cln.do",
				data:"sigungu="+sigungu+"&sido="+sido,
				type:"get",
				dataType:"json",
				//async://동기(응답이 오고난 후 success),비동기(응답이 오기전에 success)
				error:function(xhr){
				...
				$(".section_data").remove(); //지우기
					if(json_obj.data){
						$.each(json_obj.data,function(i, data){
							$("#allTab:last")
							.append("<tr class='section_data' id='clnCheck'><td>["+
...
window.onload=function(){
	...
	var mapContainer = document.getElementById('all_map'), // 지도를 표시할 div
    mapOption = { 
...
for (var i = 0; i < positions.length; i ++) {
	...
	<div class="content" style="height:800px;">
		<!-- Start Content -->
		<div id="all_list_map" style="width:1100px;height:600px;float:center;margin:0px auto;">
		<div class="search_clinic_box">
		<form name="clnFrm" action="cln_all.do" method="post">
		<input type="hidden" name="clnFlag" />
		<label>전체 지역 선택</label><br/>
			...
			<select name="sigungu" id="sigungu">
				<option value="none">-----------</option>
			<c:forEach items="${sigungu_list}" var="list">
			<option value="${ list }"><c:out value="${ list }"/></option>
			</c:forEach>
			</select>
			
			<input type="button" value="검색" class="btn" id="search_all" />
		</form>
		<div id="resultA">
		<table id="allTab">
		<tr>
			<th id="sectionTitle">지역</th>
			<th id="clnNameTitle">병원명</th>
		</tr>
		<tr id="cln38">
			<td>[서울 강남구 역삼동] </td>
			<td><a href="cln_info.do">쌍용병원38</a></td>
		</tr>
<!-- 		<tr id="none">
